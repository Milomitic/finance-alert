...
export interface SignalSnapshot {
   ...
   *  color. First-class metric since the two-score split; always emitted by
   *  the backend (legacy alerts were backfilled). Optional on the type only
   *  for defensive parsing of malformed snapshots. */
  strength?: number;
  /** Empirical historical hit-rate (0..100) → rendered as "Probabilità" in a
   *  neutral/info treatment. Always emitted by the backend (legacy alerts were
   *  backfilled); UI shows "n/d" only if it's truly absent. Optional on the
   *  type only for defensive parsing. */
  probability?: number;
