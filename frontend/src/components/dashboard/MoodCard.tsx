  ...
  return (
    <Card className={cn(cfg.bg)}>
      <CardContent className="p-4 flex flex-col items-center justify-center text-center h-full">
        <Globe2 className={cn("h-5 w-5 mb-1 opacity-60", cfg.fg)} />
        <div className={cn("text-xs font-bold uppercase tracking-wide", cfg.fg)}>Market Mood</div>
        <div className={cn("flex items-center gap-2 mt-2", cfg.fg)}>
          {cfg.icon}
          <span className="text-3xl font-bold">{cfg.label}</span>
        </div>
        <div className={cn("text-sm mt-2", cfg.fg, "opacity-80")}>
