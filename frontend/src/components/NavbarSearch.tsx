...
export function NavbarSearch() {
  ...
  const stockItems: Stock[] = useMemo(() =>
    q.trim().length > 0
    ...
    return (
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="text-[0.7059rem] text-muted-foreground">
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            <span className="text-[0.6765rem] text-muted-foreground bg-muted/60 dark:bg-muted/40 px-1.5 py-0.5 rounded">
              {s.exchange}
            </span>
            {s.sector && (
              <span className="text-[0.6765rem] text-muted-foreground bg-muted/60 dark:bg-muted/40 px-1.5 py-0.5 rounded truncate max-w-[140px]">
                {s.sector}
              </span>
            )}
          </div>
        ...
        {change != null && (
          <div className="text-right shrink-0">
            ...
            <div className="text-[0.6765rem] text-muted-foreground">oggi</div>
          </div>
        )}
      </button>
    );
    ...
    <div ref={containerRef} className="relative w-full max-w-2xl">
      ...
      {open && (
        <div id="navbar-search-results" role="listbox" aria-label="Risultati ricerca" className="absolute left-0 right-0 top-full mt-2 z-50 rounded-lg border bg-popover shadow-2xl overflow-hidden">
          ...
          {/* Footer */}
          <div className="border-t px-4 py-1.5 text-[0.6765rem] text-muted-foreground bg-muted/30 flex items-center justify-between">
            <span>
              {q.trim().length > 0
            ...
            <span>
              <kbd className="px-1 py-0.5 bg-background rounded border text-[0.6471rem]">↑↓</kbd>
              <kbd className="ml-1 px-1 py-0.5 bg-background rounded border text-[0.6471rem]">↵</kbd>
              <kbd className="ml-1 px-1 py-0.5 bg-background rounded border text-[0.6471rem]">esc</kbd>
            </span>
          </div>
        </div>
      )}
