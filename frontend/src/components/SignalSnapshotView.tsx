    ...
    <div className="space-y-4">
      {isHybrid && (
        <div className="flex items-center gap-1.5">
          <span className="inline-flex items-center gap-1 rounded-full border border-indigo-300/60 bg-indigo-50 px-2 py-0.5 text-[0.7059rem] font-semibold text-indigo-700 dark:border-indigo-700/50 dark:bg-indigo-950/40 dark:text-indigo-300">
            Ibrido
          </span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1.6fr_1fr] gap-x-6 gap-y-5">
        ...
        <div className="space-y-4 md:order-2">
          ...
          <div className="flex items-center gap-3" title={PROBABILITA_TOOLTIP}>
            <span className="w-[4.5rem] shrink-0 text-[0.7059rem] uppercase tracking-wider text-sky-700/80 dark:text-sky-300/80 font-semibold">
              Probabilità
            </span>
            <div className="flex-1 h-2 rounded-full bg-muted overflow-hidden">
              {probPct != null && (
          ...
          {cal && (
            <div className="flex items-center gap-2 flex-wrap pl-[5.5rem] -mt-2">
              {cal.tag === "coinflip" && (
                <span className="inline-flex items-center rounded-full px-1.5 py-0.5 text-[0.6765rem] font-semibold bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800/60"
                  title="Storicamente ~50/50 al netto del mercato: nessun edge direzionale dimostrato">
                  ≈50/50 storico
                </span>
              )}
              {cal.tag === "negative" && (
                <span className="inline-flex items-center rounded-full px-1.5 py-0.5 text-[0.6765rem] font-semibold bg-rose-50 dark:bg-rose-950/40 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-800/60"
                ...
                {Object.entries(factors).map(([k, v]) => {
                  ...
                  return (
                    <div key={k} className="flex items-center gap-2">
                      ...
                      <span className="w-9 text-right text-[0.7059rem] tabular-nums text-muted-foreground">{pct}%</span>
                    </div>
                  );
                })}
              </div>
              ...
              {chain.map((step, i) => {
                ...
                return (
                  <li key={`${step.date}-${i}`} className="ml-4 relative">
                    {num != null ? (
                      <span className="absolute -left-[1.72rem] top-0 h-5 w-5 rounded-full bg-slate-900 dark:bg-slate-700 text-white text-[0.6765rem] font-bold flex items-center justify-center border-2 border-background">
                        {num}
                      </span>
                    ) : (
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {badge && (
                        <span
                          className={cn(
                            "inline-flex items-center gap-0.5 rounded-full px-1.5 py-0.5 text-[0.6765rem] font-semibold leading-none",
                            badge.cls,
                          )}
                        >
      ...
      {sources.length > 0 && (
        <div className="flex items-start gap-2 text-[0.7059rem] text-muted-foreground">
