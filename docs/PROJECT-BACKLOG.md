...
| FA-030 | P1 | Dati | La finestra 52 settimane e l'intera storia su 5 timeframe su 6 | **IN PROGRESS** | Commit `633ebc2` rilasciato: sette job CI verdi, Argo `Synced/Healthy`, pod Ready sull'immagine `633ebc274d87aeb2afc3e0e2b3a30d6cb9588c85`, health esterno `ok`. La finestra mobile usa le chiusure delle ultime 52 settimane indipendentemente dal timeframe; il test parametrico sui sei timeframe e stato verificato rosso prima e verde dopo. Resta solo la verifica a schermo richiesta dal piano su `/markets/^GSPC`: 52W low diverso da Range low e mai 4.40 nei sei timeframe. |
| FA-031 | P1 | Dati | Il macro sbaglia la scala di mille volte | **IN PROGRESS** | Commit `44ce2ef` pushato. Tre difetti, una causa comune: **l'unita era nel payload e nessuno la leggeva**. (a) La scheda rende `Total Non-Farm Payrolls (thousands)` e il formatter stampava `159.1K` su 159100, gia in migliaia: sbagliava di mille volte. L'API esponeva `unit="level"` sia per una serie in migliaia sia per una in miliardi, quindi il formatter non poteva sapere. Ora `value_kind` (la forma) e `source_scale` (la scala) sono due campi, dichiarati nella mappa curata: PAYEMS migliaia, GDPC1 miliardi, RSAFS milioni. ⚠️ **Una scala ignota non viene indovinata**: si mostra il numero memorizzato senza trasformazione, come `lib/money.ts` fa senza valuta. (b) Il grafico disegnava il livello a barre da zero — sessanta barre identiche, perche la variazione mensile e un millesimo dell'altezza. Ora barre per le variazioni, linea per i livelli, asse che non forza lo zero, e il default segue il tipo di valore. La variazione e una trasformazione NOSTRA e il selettore lo dice. (c) `release_date` era costruita dal periodo osservato, quindi agosto risultava pubblicato il 1 agosto: tre campi distinti, e la pubblicazione resta `null` finche' non arriva dal calendario. Migrazione `c6b2f93a1d47`. Sei test sul formatter piu due sull'API, **confermati rossi togliendo la correzione**; fra questi l'asserzione che i nomi vecchi non ci siano piu, perche un frontend che leggesse `unit` otterrebbe `undefined` e il difetto tornerebbe in silenzio. 2034 backend, 412 frontend. Restano CI/CD e la verifica a schermo su `/macro/3`. |
...
Le diciannove voci qui sotto sono tutto quello che il backlog tiene aperto: due
in attesa di dati, due in attesa di un collaudo su dispositivo reale, tredici
aperte, una che aspetta una decisione, piu **FA-030 e FA-031 gia rilasciate**
che aspettano solo la verifica a schermo indicata nel piano.

Chiuse oggi dalla Tranche 1 del piano: **FA-032** (quote contro peso) e, in
attesa di conferma a schermo, **FA-030** (finestra 52 settimane) e **FA-031**
(scala e date del macro). Restano FA-033, le quattro basi prezzo.
