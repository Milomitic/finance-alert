...
| FA-032 | P1 | Dati | Variazione di quote stampata con l'unita del peso | **IN PROGRESS** | Implementati contratti distinti `shares_change_pct` e `portfolio_weight_delta_pp`. Il peso viene confrontato solo con il filing immediatamente precedente; se manca il peso comparabile resta `null`, senza saltare un trimestre. `AllocationBars` usa i punti percentuali quando validi, ripiega sulla variazione quote con `%` e rifiuta delta peso oltre 100pp. I test backend e frontend sono stati verificati rossi sul codice precedente e verdi dopo la correzione. Restano suite complete, push, CI/CD e verifica a schermo sul dettaglio DG. |
