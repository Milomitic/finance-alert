...
| FA-030 | P1 | Dati | La finestra 52 settimane e l'intera storia su 5 timeframe su 6 | **IN PROGRESS** | Implementata la finestra mobile di 52 settimane sulle chiusure, indipendente dal timeframe: le barre giornaliere/settimanali/mensili gia scaricate vengono filtrate; 5m/30m/1h usano una serie giornaliera annuale separata. Le etichette dichiarano la base “chiusure”. Il test parametrico sui sei timeframe e stato verificato rosso sul codice precedente e verde dopo la correzione; suite locali: backend 2028 pass/4 skip, frontend 396 pass, ruff, hooks lint e build verdi. Restano push, sette job CI, immagine in esecuzione e verifica a schermo su `/markets/^GSPC`. |
