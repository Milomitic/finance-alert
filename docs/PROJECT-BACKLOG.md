...
| FA-003 | P1 | Backup | Migrare dal Barman integrato CNPG al Barman Cloud Plugin prima di CNPG 1.31 | **DONE / PROD** | Plugin chart 0.8.0/v0.15.0 installato, `pg-1` 2/2 Ready, WAL archiviati, `ScheduledBackup pg-daily` con `method=plugin` e backup `pg-plugin-verify-20260909` **completed** in 3m34s. Il restore separato `pg-restore-fa003` chiude la verifica: cluster sano e dati corretti (stocks/alerts identici al live, ohlcv 2.476.821 contro 2.476.822 perche il live ha acquisito una barra dopo il backup; 28 tabelle, alembic `a3f71c9d2e40`, `fa_app` non superuser). ⚠️ Il cluster di restore e ancora in piedi e occupa disco: eliminarlo con la sua PVC. |
...
| FA-020 | P1 | Accessibilita | Lingua del documento, link di salto, nome del logout e titolo per rotta | **READY / LOCAL** | `lang="it"` (l'interfaccia e italiana), link "Salta al contenuto" come **primo** elemento focalizzabile, etichetta del logout da `hidden` a `sr-only` e `document.title` per rotta derivato da `NAV`. 7 test in `Layout.a11y.test.tsx`; suite frontend 289 pass. |
